...
class perlinnoise{
  ...
  setseed(seedvaule) {
    if(seedvaule==this.seedval) return;
    this.seedval = seedvaule;
    if(seedvaule > 0 && seedvaule < 1) {
      // Scale the seed out
      seedvaule *= 65536;
    }

    seedvaule = Math.floor(seedvaule);
    if(seedvaule < 256) {
      seedvaule |= seedvaule << 8;
    }

    for(var i = 0; i < 256; i++) {
      var v;
      if (i & 1) {
        v = this.p[i] ^ (seedvaule & 255);
      } else {
        v = this.p[i] ^ ((seedvaule>>8) & 255);
      }
  ...
  perlin2(x, y) {
    x=(x+this.scrollX)/this.zoomval;
    y=(y+this.scrollY)/this.zoomval;
  ...
  simplex2(xin, yin) {
    xin=(xin+this.scrollX)/this.zoomval;
    yin=(yin+this.scrollY)/this.zoomval;
...
simplex3(xin, yin, zin) {
  xin=(xin+this.scrollX)/this.zoomval;
  yin=(yin+this.scrollY)/this.zoomval;
  zin =(zin+this.scrollZ)/this.zoomval;
...
perlin2edge(x,y,zoom, borderval){
  let n = this.perlin2(x, y,zoom);
  if(borderval>=0){
    if(n>=borderval) return 1;
    return 0;
  }
  else{
    if(n<=Math.abs(borderval)) return 1;
    return 0;

  }
}

perlin3edge(x,y,z,zoom, borderval){
  let n = this.perlin3(x, y,z,zoom);
  if(borderval>=0){
    if(n>=borderval) return 1;
    return 0;
  }
...
simplex3edge(x,y,z,zoom, borderval){
  let n = this.simplex3(x, y,z,zoom);
  if(borderval>=0){
    if(n>=borderval) return 1;
    return 0;
  }
...
simplex2edge(x,y,zoom, borderval){
  let n = this.simplex2(x, y,zoom);
  if(borderval>=0){
    if(n>=borderval) return 1;
    return 0;
  }
